...
for m=1:length(EEG)
    ...
    EEG_tmp = pop_epoch( EEG,triggers, window, 'epochinfo', 'yes');
   ...
   %load components to reject, which where previously selected 
    tmp_path = sprintf('%s_ICA.mat',EEG.filename(1:7));
    comps_to_rej = load(tmp_path);
    
    EEG = eeg_checkset(EEG);
    
    %reject selected components
    EEG = pop_subcomp( EEG, comps_to_rej.comps_to_rej, 0);
    EEG = eeg_checkset(EEG);
    EEG.preprocessing = [EEG.preprocessing 'ICACleaned,'];

    %save file
    EEG.filename=sprintf('ICA_%s',EEG.filename); %change filename to prevent overwriting
    STUDY.datasetinfo(m).filename=EEG(m).filename; %change filename also in STUDY 
    EEG = pop_editset(EEG, 'setname', 'Step4_');
...
for n=1:length(EEG)
    ...
    EEG(n)=eeg_checkset(EEG(n), 'loaddata'); 
    ...
    %EEG(n) = pop_saveset(EEG(n),'filename',EEG(n).filename,'filepath',fullfile(EEG(n).filepath)); 
